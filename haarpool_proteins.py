...
!pip3 install --user --verbose --no-cache-dir torch-scatter==2.0.4+cu100 -f https://s3.eu-central-1.amazonaws.com/pytorch-geometric.com/whl/torch-1.4.0.html
!pip3 install --user --verbose --no-cache-dir torch-sparse==0.6.1+cu100 -f https://s3.eu-central-1.amazonaws.com/pytorch-geometric.com/whl/torch-1.4.0.html
!pip3 install --user --verbose --no-cache-dir torch-cluster==1.5.4+cu100 -f https://s3.eu-central-1.amazonaws.com/pytorch-geometric.com/whl/torch-1.4.0.html
...
class Net(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        x = F.relu(self.conv4(x, edge_index.to(device)))
#        x = F.relu(self.conv3(x, edge_index.to(device)))
#         x, edge_index, batch = \
#         HaarPool(x, edge_index, U, edge_index_list, num_nodes_tree, batch, batch_size, 4)
